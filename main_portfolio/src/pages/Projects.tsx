            ...
            <article key={project.title} className="project-feature">
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-center">
                <div className="lg:col-span-3">
                  <h3 className="fantasy-font text-2xl md:text-3xl text-amber-950 mb-1">{project.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((tag) => (
                      <span key={tag} className="text-xs bg-[#efe3cc] text-amber-900 px-3 py-1 rounded-full border border-amber-300/80">
                        {tag}
                      </span>
                    ))}
                  </div>
